public class latian {
    public static void main(String[] args) {
        System.out.println("---------------------------") ;
        System.out.println("***** IDENTITAS DIRI *****") ;
        System.out.println("---------------------------") ;
        System.out.println("Nama    :Ahmad Dava Khoirur Roziqin") ;
        System.out.println("Ttl     :Banyuwangi, 23 Juli 2006") ;
        System.out.println("Alamat  :Sumbergondo, Glenmore, Banyuwangi") ;
        System.out.println("Pelajar :MAN 2 Banyuwangi") ;
        System.out.println("Status  :Jomblo Fi Sabilillah") ;
        System.out.println("Hobi    :Membaca, Menulis, dan Mengaji") ;
        //tes
    }
}